...
void editor_insert_new_line(Editor *editor){
	...
	const size_t element_size = sizeof(editor->lines[0]);
	memmove(editor->lines + editor->cursor_row + 1,
		editor->lines + editor->cursor_row,
		(editor->size - editor->cursor_row) * element_size);
	memset(&editor->lines[editor->cursor_row + 1], 0, element_size);
	editor->cursor_row += 1;
	editor->cursor_col = 0;
	editor->size += 1;
}
...
void editor_backspace_before_cursor(Editor *editor){
	...
	line_backspace_before_cursor(&editor->lines[editor->cursor_row], &editor->cursor_col);
}
...
void editor_delete_over_cursor(Editor *editor){
	...
	line_delete_over_cursor(&editor->lines[editor->cursor_row], &editor->cursor_col);
}
