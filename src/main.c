...
void render_char(SDL_Renderer *render, Font *font, char c, Vec2f pos, float scale){
	const SDL_Rect dst = {
		.x = (int) floorf(pos.x),
		.y = (int) floorf(pos.y),
	...
	const size_t index = c - ASCII_DISPLAY_LOW; // remove the ascii value for space
		...
		while(SDL_PollEvent(&event)){
			switch(event.type){
				...
				case SDL_TEXTINPUT: {
					...
					const size_t freespace = BUFFER_CAPACITY - buffer_size;
					if(text_size > freespace){
						text_size = freespace;
					}
